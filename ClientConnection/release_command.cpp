...
TransponderReleaseCommand::TransponderReleaseCommand(
    const QString& name,
  ...
  connect(
      this, &TransponderReleaseCommand::releaseTransponder_signal,
...
ReturnStatus TransponderReleaseCommand::process(const QJsonObject& command) {
  ...
  emit releaseTransponder_signal(Parameters, Result, Status);
...
void TransponderReleaseCommand::generateResponse(QJsonObject& response) {
...
void TransponderReleaseCommand::reset() {
  Parameters.clear();
  Result.clear();
  Firmware->clear();
  Status = ReturnStatus::Unknown;
